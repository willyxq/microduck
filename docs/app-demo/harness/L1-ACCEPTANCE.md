...
| 5 | 按钮反应 | `…/l1-buttons.yaml`（Body-sim **down**） | 停止 / 坐下 toast；摄像头写「没有直播」；更新拒下；回退二次确认后拒切版本 |
...
| 8 | 身体孪生 | `scripts/duck-body-sim up` 后 `…/l1-body.yaml` | 互动页有 `camera-feed`；坐下 toast 写「不经 BLE」 |

场景 YAML 只打 web。iOS 走 TapBridge / XCUITest，不要用 AppleScript 点 Simulator 窗口。

不在本单：摇杆 / `teleop`、真机 BLE、第二层意图、ONNX sitstand。
